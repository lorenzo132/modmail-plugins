...
class StarboardPlugin(commands.Cog):
    """
    A starboard is a popular feature among robots that serves as a channel to messages that users of the server are funny, stupid, or both.
    With this plugin, you can add your starboard lounge Modmail bot.
    """
    ...
    async def channel(self, ctx: commands.Context, channel: discord.TextChannel):
        """
        Set which channel your starboard will be
        ...
        **Usage:**
        ...
        await ctx.send(f"Done! {channel.mention} Starboard is the lounge now!")

    @starboard.command(aliases=["setstars", "ss"])
    @checks.has_permissions(PermissionLevel.ADMIN)
    async def stars(self, ctx: commands.Context, stars: int):
        """
        Set the number of stars for which the message should appear on the starboard lounge
        ...
        **Usage:**
        ...
        await ctx.send(
            f"Completed. Now this server needs to have `{stars}` :star: to show on the starboard."
        )
    ...
    async def member(self, ctx: commands.Context, member: discord.Member):
        """
        Put a user on the blacklist so that his reaction is not recorded
        **Usage:**
    ...
    async def blacklist_channel(
        ...
        """
       Blacklist of living for messages sent to these channels will not appear on the starboard
